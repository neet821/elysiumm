...
const PORT = process.env.MINERADIO_PORT || process.env.PORT || 3000;
const HOST = process.env.MINERADIO_HOST || process.env.HOST || '0.0.0.0';
...
const musicSessionContext = new AsyncLocalStorage();
...
function verifyBlueAccessToken(token) {
  if (!BLUE_ALBUM_SECRET_KEY || !token) return null;
  const parts = String(token).split('.');
  if (parts.length !== 3 || !safeEqualText(hmacValue(parts[0] + '.' + parts[1]), parts[2])) return null;
  const payload = base64UrlJson(parts[1]);
  const userId = payload && Number(payload.user_id);
  const expiresAt = payload && Number(payload.exp);
  if (!userId || !expiresAt || expiresAt <= Math.floor(Date.now() / 1000)) return null;
  return { userId, expiresAt };
}
function verifyMusicSessionCookie(value) {
  if (!BLUE_ALBUM_SECRET_KEY || !value) return null;
  const parts = String(value).split('.');
  if (parts.length !== 3) return null;
  const userId = Number(parts[0]);
  const expiresAt = Number(parts[1]);
  if (!userId || !expiresAt || expiresAt <= Math.floor(Date.now() / 1000)) return null;
  if (!safeEqualText(hmacValue(parts[0] + '.' + parts[1]), parts[2])) return null;
  return { userId, expiresAt };
}
...
function resolveBlueMusicIdentity(req) {
  const authorization = String(req.headers.authorization || '');
  const bearer = authorization.match(/^Bearer\s+(.+)$/i);
  return verifyBlueAccessToken(bearer && bearer[1]) || verifyMusicSessionCookie(readRequestCookie(req, 'blue_music_session'));
}
function loadMusicSession(identity) {
  const key = identity && identity.userId ? 'user-' + identity.userId : 'anonymous';
  if (musicSessions.has(key)) return musicSessions.get(key);
  const baseDir = identity && identity.userId ? path.join(MUSIC_SESSION_DIR, String(identity.userId)) : path.dirname(COOKIE_FILE);
...
function currentMusicSession() { return musicSessionContext.getStore() || loadMusicSession(null); }
function getUserCookie() { return currentMusicSession().neteaseCookie; }
function getQQCookie() { return currentMusicSession().qqCookie; }
...
async function pipeAudioResponse(req, res, audioUrl, extraHeaders) {
...
const server = http.createServer((req, res) => {
  const identity = resolveBlueMusicIdentity(req);
  const session = loadMusicSession(identity);
  if (identity) {
    const value = identity.userId + '.' + identity.expiresAt + '.' + hmacValue(identity.userId + '.' + identity.expiresAt);
    res.setHeader('Set-Cookie', 'blue_music_session=' + encodeURIComponent(value) + '; Path=/; HttpOnly; SameSite=Lax; Max-Age=' + Math.max(1, identity.expiresAt - Math.floor(Date.now() / 1000)));
  }
  return musicSessionContext.run(session, async () => {
  ...
  if (pn === '/api/blue-session') {
    const state = currentMusicSession();
    sendJSON(res, {
      authenticated: !!state.userId,
      userId: state.userId,
      neteaseLoginSaved: !!state.neteaseCookie,
      qqLoginSaved: !!state.qqCookie,
    });
    return;
  }
  ...
  // ---------- 登出 ----------
  if (pn === '/api/logout') {
    try { await logout({ cookie: getUserCookie() }); } catch (e) {}
    saveCookie('');
  ...
  if (pn === '/api/internal/room/check') {
    ...
    try {
      ...
      if (!/^(netease|qq)$/.test(provider) || !/^[A-Za-z0-9_-]{1,120}$/.test(id)) {
        sendJSON(res, { playable: false, reason: 'invalid_track' }, 400);
        return;
      }
      const shared = loadMusicSession(null);
      const info = await musicSessionContext.run(shared, async () => {
        if (provider === 'qq') return handleQQSongUrl(id, mediaMid, 'exhigh');
        const loginInfo = shared.neteaseCookie
          ? await getLoginInfo()
      ...
      console.error('[RoomCheck]', err);
      sendJSON(res, { playable: false, reason: 'provider_unavailable' }, 502);
    }
    return;
  }
  ...
  if (pn === '/api/room/audio') {
    try {
      ...
      if (!info || !info.url) {
        sendJSON(res, { error: 'ROOM_TRACK_UNAVAILABLE', provider, reason: info && (info.reason || info.error), message: info && info.message }, 404);
        return;
      }
      await pipeAudioResponse(req, res, info.url, {
        'X-Blue-Album-Provider': provider,
        'X-Blue-Album-Trial': info.trial ? '1' : '0',
        'Cache-Control': 'private, no-store',
      });
    } catch (err) {
      console.error('[RoomAudio]', err);
      sendJSON(res, { error: 'ROOM_AUDIO_FAILED', message: err.message }, 502);
    }
    return;
  }
  ...
  if (pn === '/api/audio') {
    try {
      ...
      await pipeAudioResponse(req, res, audioUrl);
...
server.listen(PORT, HOST, () => {
  ...
  console.log(' 房间曲库凭据: 服务器统一保存，浏览器不可读取');
  console.log('======================================================');
});
