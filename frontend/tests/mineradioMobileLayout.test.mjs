...
const visualPersistence = read('mineradio/public/js/modules/02-visual/04-visual-settings-persistence.js')
...
test('desktop playback does not restore an empty void preset without a custom background', () => {
  ...
  assert.match(homeWallpaper, /backgroundAlbumCover|backgroundOpacity|windowBackgroundOpacity|backgroundGlassOpacity|backgroundImage|backgroundMedia/)
  assert.match(homeWallpaper, /return fxDefaults\.preset/)
  assert.match(homeWallpaper, /playbackVisualPresetFallbackActive = savedPlaybackVisualPreset === 3 && targetPreset !== savedPlaybackVisualPreset/)
  assert.match(homeWallpaper, /resolvePlaybackVisualPresetForPlayback\(targetPreset\)/)
  assert.match(visualPersistence, /playbackVisualPresetFallbackActive/)
  assert.match(visualPersistence, /\? playbackVisualPreset/)
})
