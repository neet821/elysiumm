...
describe('ArticleFlowHome', () => {
  ...
  it('fixes the homepage chrome and joins the mobile drawer to it', () => {
    ...
    expect(css).toMatch(/\.legacy-old-home--flat \.home-sidebar\.home-sidebar--drawer-open\s*> \.sidebar-section--records\.sidebar-section--has-overflow\s*\{[^}]*block-size:\s*min\(17rem,\s*38dvh\);/s)
    ...
    expect(css).toMatch(/@media \(min-width:\s*801px\)[\s\S]*?\.home-nav__sidebar-toggle\s*\{[^}]*display:\s*none;/s)
