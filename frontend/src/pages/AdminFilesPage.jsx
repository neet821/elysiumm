...
export default function AdminFilesPage() {
  ...
  const loadSync = useCallback(async (nextPath = '') => {
    ...
    try {
      ...
      setSync({ status: 'online', path: browse.data?.path || nextPath, items: Array.isArray(browse.data?.items) ? browse.data.items : [] })
    } catch (reason) {
  ...
  const goTo = (item) => {
    ...
    const next = [sync.path, item.path].filter(Boolean).join('/')
    loadSync(next)
  }
  ...
  return <section className="admin-files-page">
    ...
    <div className="admin-file-cards">
      <article className={`admin-file-card${syncOnline ? '' : ' admin-file-card--disabled'}`}>
        ...
        <p className="admin-file-card__label">READ ONLY / FRP</p>
        <h3>文件同步</h3>
        <p>实时浏览并下载 <code>/home/neet821/Public</code>，服务器不写回本机。</p>
        ...
        <div className="admin-file-browser" aria-disabled={!syncOnline}>
          ...
          {syncOnline && sync.items.map((item) => <button className="admin-file-browser__item" type="button" disabled={!syncOnline} key={item.path} onClick={() => item.path.endsWith('/') ? goTo(item) : downloadSync(item)}>{item.path.endsWith('/') ? <FolderSync size={15} /> : <FileDown size={15} />}<span>{item.name}</span>{!item.path.endsWith('/') && <Download size={14} />}</button>)}
          {sync.status === 'online' && !sync.items.length && <p className="admin-empty">目录为空。</p>}
      ...
      <article className="admin-file-card">
        ...
        <p className="admin-file-card__label">ANONYMOUS / 5 MIN IDLE</p>
        <h3>文件中转</h3>
        <p>在此页直接上传，成功后生成匿名上传下载链接，单个链接总量最多 2GB，五分钟无活动自动销毁。</p>
        ...
        <div className="admin-transfer-list">{transfers.map((item) => <div className="admin-transfer-row" key={item.id}><span><strong>{formatSize(item.total_bytes)} / {formatSize(item.max_bytes)}</strong><small>过期：{formatTransferDate(item.expires_at)}</small></span><button type="button" onClick={() => deleteTransfer(item.id)} aria-label="销毁中转链接"><Trash2 size={15} /></button></div>)}</div>
      </article>
    </div>
  </section>
}
