...
export default function TransferPage() {
  ...
  return <section className="transfer-page"><header><p className="rooms-hub__kicker">TRANSFER</p><h1>文件中转</h1><p>链接仅在五分钟无活动后失效，页面轮询不会延长有效期。</p></header>{error && <p className="admin-inline-error" role="alert">{error}</p>}{data && <><div className="transfer-page__meta"><span>{formatExpiry(data.expires_at)}</span><span>{formatSize(data.total_bytes)} / {formatSize(data.max_bytes)}</span></div><label className="transfer-page__upload"><Upload size={20} />{busy ? '上传中…' : '选择文件上传'}<input type="file" onChange={upload} disabled={busy} /></label><div className="transfer-page__files">{data.files.map((item) => <a key={item.id} href={item.download_url}><Download size={16} />{item.name}<small>{formatSize(item.size)} · {item.sha256}</small></a>)}</div></>}</section>
}
