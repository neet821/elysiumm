...
class MusicRoomRebuildTest(unittest.TestCase):
    ...
    def test_like_count_is_deduped_and_controls_next_order(self):
        ...
        self.assertTrue(music_service.like_queue_item(self.db, self.room, self.host, second)["liked"])
        self.assertFalse(music_service.like_queue_item(self.db, self.room, self.host, second)["liked"])
        self.assertTrue(music_service.like_queue_item(self.db, self.room, self.one, second)["liked"])
        self.assertTrue(music_service.like_queue_item(self.db, self.room, self.two, second)["liked"])
        self.assertTrue(music_service.like_queue_item(self.db, self.room, self.one, first)["liked"])
        payload = next(item for item in music_service.queue_payload(self.db, self.room.id) if item["id"] == second.id)
        self.assertEqual(payload["like_count"], 2)
        self.assertEqual(payload["liked_by_user_ids"], [self.one.id, self.two.id])
