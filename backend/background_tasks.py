...
async def cleanup_task(interval_minutes: int = 5, empty_timeout_minutes: int = 10):
    ...
    """
    logger.info(
        f"空房间清理任务启动 - 检查间隔:{interval_minutes}分钟, 超时阈值:{empty_timeout_minutes}分钟"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--interval", type=int, default=5, help="检查间隔（分钟），默认5分钟"
    )
    parser.add_argument(
        "--timeout", type=int, default=10, help="空房间超时时间（分钟），默认10分钟"
    )
