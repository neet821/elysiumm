...
function addBox(group, registry, name, size, position, materialKey = 'frame') {
...
export function createBayWindow({ materials, scenery = DEFAULT_SCENERY, textureLoader } = {}) {
  ...
  addBox(group, materials, 'window-top-frame', new THREE.Vector3(8.85, 0.22, 0.22), new THREE.Vector3(0, 5.52, -4.15))
  addBox(group, materials, 'window-bottom-frame', new THREE.Vector3(8.85, 0.26, 0.48), new THREE.Vector3(0, 0.98, -3.98))
  addBox(group, materials, 'window-left-frame', new THREE.Vector3(0.22, 4.72, 0.22), new THREE.Vector3(-4.32, 3.25, -4.15))
  addBox(group, materials, 'window-right-frame', new THREE.Vector3(0.22, 4.72, 0.22), new THREE.Vector3(4.32, 3.25, -4.15))
  addBox(group, materials, 'window-mullion-left', new THREE.Vector3(0.16, 4.5, 0.18), new THREE.Vector3(-1.46, 3.25, -4.04))
  addBox(group, materials, 'window-mullion-right', new THREE.Vector3(0.16, 4.5, 0.18), new THREE.Vector3(1.46, 3.25, -4.04))
  ...
  async function setScenery(nextScenery) {
    ...
    const texture = await loadSceneryTexture(resolvedScenery, textureLoader)
    ...
    sceneryMaterial.map = texture
    sceneryMaterial.color.set('#ffffff')
    sceneryMaterial.needsUpdate = true
    group.userData.scenery = resolvedScenery
    group.userData.sceneryUrl = resolveSceneryUrl(resolvedScenery)
    return true
  }
  ...
  return {
    ...
    dispose() {
      ...
      interactionProxy.geometry.dispose()
